...
public class PasswordIdentify {
    ...
    public static int identify(String str) {
        ...
        if (StringUtils.isBlank(str)) throw new NullPointerException("错误：密码不能为空!");
        //字符串不符合要求，丢一个 IllegalArgumentException 异常
        if (!StringUtils.containsOnly(str, matchAll)) throw new IllegalArgumentException("错误：密码只能为数字、字母或符号!");
        //字符串每有一种类型的字符，安全度+1
    ...
    private static void printLevel(String str, int level) {
        System.out.println("\"" + str + "\" 的安全度为 " + level);
    }
}
